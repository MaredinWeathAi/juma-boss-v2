...
router.post('/orders/quick', (req, res) => {
    try {
        const bakeryId = getBakeryId(req.user.id);
        ...
        const product = db.prepare('SELECT * FROM products WHERE id = ? AND bakery_id = ?').get(productId, bakeryId);
        if (!product) {
        ...
        const transaction = db.transaction(() => {
            ...
            let customer = db.prepare('SELECT id FROM customers WHERE bakery_id = ? AND name = ? LIMIT 1').get(bakeryId, customerName);
            let customerId;
            if (customer) {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(orderId, bakeryId, customerId, orderNumber, 'pending', total, deliveryDate || null, 'pickup', 'unpaid', now, now);
            // Create order item
            db.prepare(`
