  ...
  `);
    ...
    for (const tierTarget of bakerTargets) {
        ...
        for (let i = 0; i < tierTarget.count; i++) {
            ...
            insertPaymentMethodStmt.run(paymentMethodId, bakeryId, 'pix', 'PIX - CPF', 1, JSON.stringify(paymentDetails), bakerCreatedAt);
            ...
            // Create subscription
            insertSubscriptionStmt.run(subscriptionId, bakeryId, tierTarget.tier, subStatus, tierPrices[tierTarget.tier], startDate.toISOString(), nextBillingDate.toISOString(), subStatus === 'trialing' ? new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString() : null, nextBillingDate.toISOString(), 'monthly', tierTarget.tier !== 'free' ? paymentMethodId : null, 0, startDate.toISOString());
            ...
            for (let idx = 0; idx < steps.length; idx++) {
                ...
                insertOnboardingStmt.run(uuidv4(), bakeryId, steps[idx], completed ? 1 : 0, completed ? new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString() : null);
            }
            // Generate products
            ...
            for (let p = 0; p < productCount; p++) {
                ...
                insertProductStmt.run(productId, bakeryId, product.name, `${product.name} artesanal, feito com ingredientes selecionados`, product.category, price, Math.round(price * 0.35 * 100) / 100, Math.round(Math.random() * 120 + 15), 1, new Date(Date.now() - Math.random() * 60 * 24 * 60 * 60 * 1000).toISOString());
            ...
            for (let c = 0; c < customerCount; c++) {
                ...
                insertCustomerStmt.run(customerId, bakeryId, customerName, `${customerName.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\s/g, '.')}@email.com`, `+55${custArea}9${Math.floor(Math.random() * 90000000 + 10000000)}`, Math.random() > 0.8 ? 1 : 0, new Date(Date.now() - Math.random() * 120 * 24 * 60 * 60 * 1000).toISOString());
            }
            // Generate orders
            ...
            for (let month = maxMonths - 1; month >= 0; month--) {
                ...
                for (let o = 0; o < ordersThisMonth; o++) {
                    ...
                    insertOrderStmt.run(orderId, bakeryId, customerId, orderNumber, status, total, deliveryDate.toISOString(), Math.random() > 0.3 ? 'pickup' : 'delivery', paymentStatus, orderDate.toISOString(), orderDate.toISOString());
                    // Order items
                    ...
                    if (paymentStatus === 'paid') {
                        ...
                        insertPaymentStmt.run(uuidv4(), bakeryId, orderId, customerId, total, method, 'completed', method === 'pix' ? `PIX-${Math.floor(Math.random() * 1000000)}` : null, orderDate.toISOString());
                    }
                }
            }
            ...
            if (tierTarget.tier !== 'free') {
                for (const ingredient of ingredients) {
                    ...
                    insertIngredientStmt.run(ingredientId, bakeryId, ingredient.name, ingredient.unit, ingredient.cost, Math.random() * 100 + 50, 20, 'baking', now, now);
                ...
                for (const product of createdProducts) {
                    ...
                    for (const recipeItem of template) {
                        ...
                        insertRecipeItemStmt.run(uuidv4(), product.id, ingredient.id, adjustedQty, recipeItem.batch, now);
                    }
                }
            }
            ...
            if (tierTarget.tier !== 'free') {
                ...
                const employeeCount = tierTarget.tier === 'starter'
                ...
                for (let e = 0; e < employeeCount; e++) {
                    ...
                    insertEmployeeStmt.run(uuidv4(), bakeryId, employeeNames[e % employeeNames.length], `${employeeNames[e % employeeNames.length].toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')}${bakerIndex}@email.com`, `+55${empArea}9${Math.floor(Math.random() * 90000000 + 10000000)}`, ['baker', 'decorator', 'manager'][Math.floor(Math.random() * 3)], Math.round((Math.random() * 8 + 12) * 100) / 100, 1, now);
                }
            }
            // Notifications
            insertNotificationStmt.run(uuidv4(), userId, 'info', 'Welcome to Juma Boss', 'Get started by adding your first products and customers', 1, new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString());
            if (tierTarget.tier === 'free') {
                insertNotificationStmt.run(uuidv4(), userId, 'upgrade', 'Unlock more features', 'Upgrade to Starter to access inventory management and team features', 0, now);
            }
        }
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
    insertAnnouncementStmt.run(uuidv4(), adminId, 'Welcome to Juma Boss v2', 'We are excited to announce the latest version of Juma Boss with improved performance and new features!', null, 1, now);
    insertAnnouncementStmt.run(uuidv4(), adminId, 'New Analytics Dashboard Available', 'Check out our new advanced analytics dashboard for better insights into your business.', 'pro,enterprise', 1, new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString());
